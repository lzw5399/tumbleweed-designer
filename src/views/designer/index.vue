...
export default {
  ...
  data() {
    return {
      ...
      rules: {
        icon: [
          {
            required: false, message: '请输入流程图标', trigger: 'blur'
          }
        ],
        name: [
          {
        ...
        tpls: [
          {
            required: false, message: '请选择模版', trigger: 'blur, change'
          }
        ],
        structure: [
          {
  ...
  methods: {
    ...
    handleEdit(row) {
      ...
      getProcessDefinition(row.id).then(response => {
        this.ruleForm = {
          ...
          classify: response.data.classify_id,
    ...
    verifyProcess(structureValue) {
      for (const r of structureValue.nodes) {
      ...
      for (const e of structureValue.edges) {
